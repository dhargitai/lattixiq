...
const InteractiveWrapper = (args: { items: NavItem[] }) => {
...
const WithContentWrapper = (args: { items: NavItem[] }) => {
  const [activeItem, setActiveItem] = useState("home");
  ...
  return (
    <div className="relative h-screen bg-background">
      <div className="h-full pb-16 overflow-auto">{content[activeItem]}</div>
      <BottomNavigation
        {...args}
        activeItem={activeItem}
        onItemClick={(item) => setActiveItem(item.id)}
...
export const WithContent: Story = {
  ...
  render: (args) => <WithContentWrapper {...args} />,
};
