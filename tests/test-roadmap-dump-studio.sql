-- ========================================
-- LETTIXIQ TEST ROADMAP DATA DUMP (Supabase Studio Version)
-- ========================================
...
-- 2. Change the email in line 18 below to match your test user's email
-- 3. Copy this entire script and paste it into Supabase Studio SQL Editor
-- 4. Run the script
...
DECLARE
    ...
    -- ========================================
    v_target_email text := 'test@example.com';  -- <-- CHANGE THIS EMAIL
...
BEGIN
    ...
    IF v_user_id IS NULL THEN
        RAISE EXCEPTION 'User with email % not found. Please sign up this user first!', v_target_email;
    END IF;
    
    RAISE NOTICE 'Found user % with ID: %', v_target_email, v_user_id;
    ...
    RAISE NOTICE '';
    RAISE NOTICE '========================================';
    RAISE NOTICE 'Successfully created test data for user: %', v_target_email;
...
-- Run this query separately after the insert to check:
/*
SELECT 
    r.id as roadmap_id,
    r.goal_description,
